...
function discoverUnsupported(platform, options = {}) {
  const checkedAt = new Date().toISOString();
  return {
    generatedAt: checkedAt,
    ...
    candidates: buildConfiguredCurrentUserSources(options, checkedAt),
    processEvidence: [],
    serviceEvidence: [],
    otherDashboardInstances: [],
    counts: {
...
function buildProcessOnlySources(processes, users, currentUser, checkedAt) {
  ...
  for (const proc of processes) {
    if (!["codex", "claudeCode", "copilot", "gemini", "ollama"].includes(proc.providerId)) continue;
...
function buildServiceOnlySources(services, checkedAt) {
  return services
    .filter((service) => ["codex", "claudeCode", "copilot", "gemini", "ollama"].includes(service.providerId))
    .map((service) => ({
...
function buildConfiguredCurrentUserSources(options, checkedAt) {
  const currentUser = getCurrentUser();
  const configured = [];
  const homes = options.codexHomes || [];
  for (const home of homes) {
    configured.push(configuredSource("codex", currentUser, checkedAt, [
...
function configuredSource(providerId, owner, checkedAt, paths) {
  return {
    id: sourceId(providerId, owner.uid, paths.map((entry) => entry.path)),
    providerId,
    kind: "usage_dir",
