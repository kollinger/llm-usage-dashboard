...
function eventDedupeKey(event) {
  ...
  if (evidence.sourceEventSha256) {
    return `${provider}:source-event:${evidence.sourceEventSha256}`;
  }
...
function aggregateUsageEvents(rawEvents, options = {}) {
  ...
  return {
    ...
    slots: buildRecentSlots(deduped.events, {
      now,
...
function buildUsageSlots(events, startMs, endExclusive, slotMs) {
  ...
  for (const event of Array.isArray(events) ? events : []) {
    ...
    if (event.model) {
      ...
      addUsage(sourceTotals.modelMap.get(event.model).allTime, event.usage || {});
    }
  }
...
function normalizeEvidence(evidence) {
  ...
  return {
    sourceEventSha256: normalizeSha256(value.sourceEventSha256 || value.rawLineSha256 || value.rawObjectSha256),
    rawLineSha256: normalizeSha256(value.rawLineSha256),
    rawObjectSha256: normalizeSha256(value.rawObjectSha256),
