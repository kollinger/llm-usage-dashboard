...
function openAiPlanProviderSnapshot(planType, fetchedAt, source) {
  return {
    ...
    sourceType: "browser",
    sourceUrl: OPENAI_CHATGPT_BILLING_URL,
...
async function fetchOpenAiDashboardSnapshot(cookieHeader) {
  const payloads = {};
  for (const url of [OPENAI_IDENTITY_API_URL, OPENAI_CHATGPT_BILLING_URL, OPENAI_DASHBOARD_API_URL, OPENAI_DASHBOARD_USAGE_URL]) {
    try {
