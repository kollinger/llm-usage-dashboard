...
The GPT accounts panel combines the ChatGPT accounts found in every configured Codex home with OpenCode OpenAI OAuth identities found in each configured OpenCode data profile. **Re-scan accounts** reads the current 5-hour and weekly Codex quota separately for every reachable account. The panel shows quota coverage as a fraction and marks an account's limit as unavailable instead of treating identity-only detection as complete. Previously observed identities and their last measured quota remain visible as history. Additional profiles can be supplied with `LLM_USAGE_CODEX_HOMES`, `LLM_USAGE_OPENCODE_DATA_DIRS`, and `LLM_USAGE_OPENCODE_DB_FILES` using the OS path delimiter.

Codex has one active cached login per `CODEX_HOME`; the dashboard starts an isolated read-only Codex app-server for each configured home so those profiles can be refreshed without replacing one another. To keep several accounts live at once without copying credentials, create one directory per account under `~/.codex-accounts` and run `CODEX_HOME=~/.codex-accounts/<profile> codex login` once for each directory. Changing only the account in a normal ChatGPT browser tab does not replace a cached Codex login. While the desktop backend is running, it watches the configured Codex and OpenCode `auth.json` files and captures a fresh sanitized account/limit snapshot shortly after a login changes, even when the dashboard window is closed. OpenCode quota is read with the active OpenAI OAuth access token already stored in that profile's `auth.json`; the dashboard never sends the refresh token, never writes credentials, and stores only the normalized quota snapshot. If the OpenCode access token is no longer accepted, use that profile in OpenCode once and re-scan.
