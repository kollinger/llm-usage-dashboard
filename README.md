...
The GPT accounts panel combines the ChatGPT accounts found in every configured Codex home with every OpenCode OpenAI OAuth identity found in the built-in OpenCode auth store and supported OpenCode multi-account stores. **Re-scan accounts** reads the current 5-hour and weekly Codex quota separately for every reachable account. The panel shows quota coverage as a fraction and marks an account's limit as unavailable instead of treating identity-only detection as complete. Previously observed identities and their last measured quota remain visible as history. Additional profiles or auth stores can be supplied with `LLM_USAGE_CODEX_HOMES`, `LLM_USAGE_OPENCODE_DATA_DIRS`, `LLM_USAGE_OPENCODE_DB_FILES`, and `LLM_USAGE_OPENCODE_AUTH_FILES` using the OS path delimiter.
